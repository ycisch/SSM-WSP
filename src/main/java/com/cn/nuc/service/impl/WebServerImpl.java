...
import com.cn.nuc.dao.WorktableMapper;
...
public class WebServerImpl implements WebServer {
	...
	public void displayStudent() {
		...
		for (Groupheader groupheader : groupheader_list) {
			...
			RelationDate.TEAMLEADER_NUM[groupheader.getAnumber().intValue()] = groupheader.getCount().intValue();
		}
		...
		for (Worktable worktable : work_list) {
			...
			for(int i = 0; i < len; i++) {
				...
				if(RelationDate.TEAMLEADER_NUM[num] != 0) {
		...
		//第八步、对组长信息保存
		for (Groupheader groupheader : groupheader_list) {
			Long count = (long) RelationDate.TEAMLEADER_NUM[groupheader.getAnumber().intValue()];
